  ...
  try {
    ...
    while (!done) {
      ...
      if (value) {
        ...
        // Perplexity is OpenAI-compatible SSE
        // We need to proxy it to client
        // But we want to modify it? Or just pass through?
        // Client expects specific format? "data: { type: 'content', text: '...' }"
        // Perplexity sends "data: { choices: [{ delta: { content: '...' } }] }"
