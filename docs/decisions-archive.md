# Decisions Archive（2026-07〜08の完了済み記録）

`.claude/rules/decisions-log.md` から退避した、2026年7〜8月の意思決定の詳細記録。**毎セッションの自動読み込み対象外**（`.claude/rules/` の外に置いている）。

- ここにある決定のうち、再発防止に必要なルールはすべて `decisions-log.md` の「廃止・先祖返り禁止」テーブルに1行ずつ残してある。**有効性は失っていない**（退避したのは経緯・実測値・調査手順などの詳細のみ）。
- 該当領域（SEO・プリレンダリング・sitemap・404・AdSense・コラム記事の書式・カウンターの＋/−の見た目など）を触るときは、該当エントリを読んでから着手すること。
- 後日の決定で上書きされた箇所は、本文中に「※」または「（2026-09-xxに…）」の形で注記してある。
...
- **現行仕様**: 実装例は `src/data/machines/haihai-siosai.ts` / `haihai-siosai2.ts` / `last-utopia.ts`（`specialAdvice`）、アイムジャグラーEXの近似ラベル特例（`specs.approximationLabelOverride`）は**2026-09-22に廃止**し、`src/utils/approximation-label.ts` の同値まとめ処理に置き換えた。詳細は `architecture.md` §5参照。
