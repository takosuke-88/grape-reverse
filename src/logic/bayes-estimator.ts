import type { MachineConfig, UserInputs, EstimationResult, DiscriminationElement } from '../types/machine-schema';
...
export function calculateEstimation(
  ...
  const logLikelihoods = settings.map(setting => {
    ...
    config.sections.forEach(section => {
      section.elements.forEach(element => {
        ...
        // 判別重要度で重み付け
        const weight = element.discriminationWeight || 1.0;
  ...
  const maxLogLikelihood = Math.max(...logLikelihoods.map(l => l.logLikelihood));
 ...
 * 最も可能性の高い設定を取得
 */
export function getMostLikelySetting(results: EstimationResult[]): EstimationResult {
