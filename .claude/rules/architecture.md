...
- **フロントマター**: `title` / `description` / `date`（`YYYY-MM-DD`。公開日）/ `updatedAt`（`YYYY-MM-DD`。更新日。新規作成時は`date`と同一値で初期化し、リライト時のみ`updatedAt`を更新する。`date`自体は公開後に変更しない）/ `tags: string[]` / `draft: boolean` / `showRelatedColumns?: boolean`（**2026-09-01以降は未参照**。関連記事の表示可否は `getRelatedColumns()` が返す候補件数のみで決まり、0件ならセクション自体が出ない。型定義と全記事のフロントマターには残っているが、値を変えても表示は変わらない。将来「関連は出せるが編集判断で出したくない」記事の抑制フラグとして再利用する余地を残すための温存）。単一の `category` フィールドは持たない（パチンコ/パチスロどちらとも言えない記事があるため `tags` の自由記述で表現する）。
...
- **関連記事の選定**: `column-content.ts` の `getRelatedColumns(slug, limit)` が担う。固有タグ（IDF重み付け）・機種CTA・カテゴリCTAの一致を根拠に最大3件を返し、**根拠が無ければ0件**（`RelatedColumns` は null を返して非表示）。`column`/`news`/`pachinko`/`pachislot` は全記事に付く形式タグのため `GENERIC_RELATED_TAGS` として根拠から除外する。**最新記事によるフォールバックは実装しない**（詳細は `decisions-log.md` 2026-09-01）。
