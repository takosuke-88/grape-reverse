...
- **API**: `const { confirm, confirmDialog } = useConfirmDialog();` で、`const ok = await confirm({ title, message, warning?, confirmLabel }); if (!ok) return;` の形。`{confirmDialog}` をページ直下に置く。**非同期化が一番危ない箇所**（キャンセルしたのに消えると最悪の事故）なので、3ページ×4挙動（キャンセル／Esc／背景タップ／実行）を実機で確認済み。
- **UI仕様**: 左＝キャンセル（グレー）、右＝実行（`bg-red-600`。`coding-style.md` §2 の「破壊的操作は赤」に従う）。背景タップとEscはキャンセル扱い。`warning` はアンバーのコールアウトで強調（赤はボタン専用なので文字色には使わない）。ボタンは `min-h-[48px]`。
